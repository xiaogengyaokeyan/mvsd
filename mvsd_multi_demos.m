...
% dt = 0.001;
% x(1,1) = 0;
% y(1,1) = (demo1_y(1)+demo2_y(1))/2;
% x_end = demo_x(end) + 0.0;
% y_end = (demo1_y(end)+demo2_y(end))/2 - 0.0;
% R_search = 100;
% steps = 1000;
...
for i = 1:steps %length(t)-1
    ...
    ydd(i+1) = kp_1*y_a - kv_1*yd(i) + k_e*(kp_2*y_r1 - kv_2*yd(i) + kp_3*y_r2 - kv_3*yd(i) + kp_4*y_t1 + kv_4*yd(i) + kp_5*y_t2 + kv_5*yd(i)) + Fe; % e+r+t
%     ydd(i+1) = kp_1*y_a - kv_1*yd(i) + k_e*(kp_2*y_r1 - kv_2*yd(i) + kp_3*y_r3 - kv_3*yd(i) + kp_4*y_t1 + kv_4*yd(i) + kp_5*y_t3 + kv_5*yd(i)) + Fe; % e+r+t
    ...
    xdd(i+1) = kp_1*x_a - kv_1*xd(i) + k_e*(kp_2*x_r1 - kv_2*xd(i) + kp_3*x_r2 - kv_3*xd(i) + kp_4*x_t1 + kv_4*xd(i) + kp_5*x_t2 + kv_5*xd(i)) + Fe; % e+r+t
...
hold on;
plot(demo_x, demo2_y,'r','LineWidth',1.0);
...
plot(x_end, y_end, 'r*');
% plot(0.0, -0.4);
% legend('demonstration\_1', 'demonstration\_2', 'repro1,kp\_2=50000,kp\_3=1000','repro2,kp\_2=30000,kp\_3=1000','repro3,kp\_2=10000,kp\_3=1000',...
...
title('Reproduction of Multi-Demostrations with MVSD 1');
